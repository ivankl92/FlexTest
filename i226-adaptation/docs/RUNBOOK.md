...
| `portState FAULTY`, usually appearing at high background load, and every later point skipped | ptp4l timed out waiting for the TX timestamp of its own PTP frame (`journalctl -u ptp4l@enp1s0` shows *timed out while polling for tx timestamp*) and faulted the port. It does not reliably clear itself | `run_measurement.sh` now restarts the unit once automatically. By hand: `sudo systemctl reset-failed ptp4l@enp1s0 && sudo systemctl restart ptp4l@enp1s0`. If it recurs, raise `tx_timestamp_timeout` in `/etc/linuxptp/gPTP.cfg` (default here 200 ms) and reduce `BG_STREAMS` |
